...
function saveChanges() {
  ...
  if (checkedCheckbox) {
    ...
    document.getElementById('addressForm').action = `/api/checkoutAddressChange?id=${selectedAddressId}`;

    // Optional: Log the updated form action
    console.log('Updated Form Action:', document.getElementById('addressForm').action);
      ...
      success: function (res) {
        ...
        if (res.Message == 'cash on delivery') {
          ...
          var options = {
            ...
            "callback_url": "http://localhost:8880/api/rayzorPayment-verification",
            "prefill": {
              "name": "Gaurav Kumar",
        ...
        if (xhr.status === 404) {
          ...
          var PaymentFormValidation = document.getElementById('paymentFormValidation')
  ...
  if (errMessage.length === 0 && a!=0) {
    ...
    $.ajax({
      ...
      success: function (response) {
        ...
        Swal.fire(`Congratulations! You received a ${discountPercentage}% discount`);
...
function discount(ss,s) {
  ...
  document.getElementById("discountCoupon-display").innerHTML = CoupendiscountTotal;
  document.getElementById("total-display").innerHTML = calculatedTotal - CoupendiscountTotal;
  document.getElementById("totalPrice").value = Number(calculatedTotal - CoupendiscountTotal);
